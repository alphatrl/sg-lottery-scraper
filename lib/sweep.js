...
export default async function sweep(browser) {
  ...
  const results = await page.evaluate( () => {
    ...
    return items.map( (item) => {
      const drawNo = item.querySelector('.drawNumber').textContent.trim().split(" ")[2]
      const drawDate =  Date.parse(item.querySelector('.drawDate').textContent.trim())
      
      const winning = [
        item.querySelector('.valueFirstPrize').textContent.trim(),
        item.querySelector('.valueSecondPrize').textContent.trim(),
        item.querySelector('.valueThirdPrize').textContent.trim()
      ]
      ...
      for (let i = 0; i < jackpotLucky.length; i ++) {
        ...
        for (let j = 0; j < nodes.length; j ++) {
          nodeList.push(nodes[j].textContent)
        }
