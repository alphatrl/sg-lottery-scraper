...
async function getFourD() {
  ...
  let tables = await driver.findElements(By.className("tables-wrap"))
...
async function getToto() {
  ...
  while (count < 3) {
    ...
    if (count % 2 == 0) {
      ...
      await driver.sleep(3000) // pause for 3 sec to finish animation
    }
  }
  driver.quit()
  return totoList
...
getToto().then((list) => console.log(list))
